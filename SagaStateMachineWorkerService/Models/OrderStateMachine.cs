    ...
    public class OrderStateMachine : MassTransitStateMachine<OrderStateInstance>
    {
        ...
        public OrderStateMachine()
        {
            ...
            During(OrderCreated, When(StockReservedEvent)
                ...
                .Send(new Uri($"queue:{RabbitMqConstants.PaymentStockReservedRequestEventQueueName}"), context => new StockReservedRequestPaymentEvent(context.Instance.CorrelationId)
                {
                ...
                When(StockNotReservedEvent).TransitionTo(StockNotReserved).Publish(context => new OrderRequestFailedEvent() { OrderId = context.Instance.OrderId, Reason = context.Data.Reason }));

            During(StockReserved, 
                When(PaymentCompletedEvent)
                .TransitionTo(PaymentCompleted)
                .Publish(context => new OrderCreatedRequestCompletedEvent { OrderId = context.Instance.OrderId })
                ...
                When(PaymentFailedEvent)
                    ...
                    .Send(new Uri($"queue:{RabbitMqConstants.StockRollBackMessageQueueName}"), context => new StockRollBackMessage { OrderItems = context.Data.OrderItems })
